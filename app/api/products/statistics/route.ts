// app/api/products/statistics/route.ts
...
export async function GET(request: NextRequest) {
    ...
    if (reportType === 'prices') {
        path = '/products/statistics/prices';
    } else {
        const queryParams = new URLSearchParams();
        queryParams.append('groupBy', groupBy);
        const queryString = queryParams.toString();
        path = `/products/statistics/categories${queryString ? `?${queryString}` : ''}`;
    }
    ...
    try {
        ...
        console.log(`Statistics retrieved successfully - type: ${reportType || 'categories'}`);
