...
import { CreateSaleFormValues } from "@/types/sales";
import { X, CreditCard, DollarSign, Smartphone, Banknote, Printer } from "lucide-react";
...
export function PaymentDialog({ store, employee }: PaymentDialogProps) {
    ...
    const { subtotal, totalDiscount, total, tax, totalWithTax } = calculateTotals();
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            // Create sale via API
            const response = await SalesAPI.createSale(accessToken, saleData);
            ...
            toast.success("Payment processed successfully!", {
                description: `Sale #${response.sale.id.slice(-8).toUpperCase()} completed`,
            });
    ...
    const paymentMethods = [
        ...
        { id: "CREDIT_CARD" as PaymentMethod, label: "Card", icon: CreditCard },
        { id: "DEBIT_CARD" as PaymentMethod, label: "Debit Card", icon: CreditCard },
        { id: "MOBILE_MONEY" as PaymentMethod, label: "Mobile", icon: Smartphone },
        { id: "BANK_TRANSFER" as PaymentMethod, label: "Transfer", icon: DollarSign },
    ];
