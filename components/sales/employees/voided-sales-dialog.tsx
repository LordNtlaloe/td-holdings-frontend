// components/sales/dialogs/void-sale-dialog.tsx
'use client';
...
export function VoidSaleDialog({
    ...
    const handleVoid = async () => {
        ...
        try {
            ...
            toast("Success",{
                description: 'Sale has been voided successfully',
            });
            onVoidSuccess();
            onOpenChange(false);
            setReason('');
        } catch (error: any) {
            setError(error.message || 'Failed to void sale');
            toast("Error",{
                description: error.message || 'Failed to void sale',
            });
        } finally {
            setLoading(false);
        }
